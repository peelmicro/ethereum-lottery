...
const deploy = async () => { // We need to create a function to use async
  ...
  // the contract
  inbox = await new web3.eth.Contract(JSON.parse(interface)) // Teaches web3 about what methods the contract has
    .deploy({ data: bytecode }) // Tells web3 that we want to deploy a copy of this contract: We don´t need to initialize the contract with any value
